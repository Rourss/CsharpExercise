...
namespace QiuXingMing.DBUtility
{
    ...
    {
        static SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["MyStockSale"].ConnectionString);
        /// <summary>
        ...
        {
            //1.创建连接对象
            //SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["MyStockSale"].ConnectionString);
            //2.创建命令对象(不用创建，从参数传入)
        ...
        public static SqlDataReader ExcuteReader(SqlCommand cmd)
        {
            ...
            { //可能出现异常的语句
                ...
                conn.Open();
                //4.执行命令
                SqlDataReader reader = cmd.ExecuteReader(CommandBehavior.CloseConnection); //创建读取器用来存放执行SQL语句后的结果，如果关闭读取器，自动关闭连接对象
                return reader;
            }
            catch (Exception ex)
            { //抛出异常
        ...
        public static SqlTransaction ExcuteSQLTransaction(SqlCommand cmd)
        {
            ...
            { //可能出现异常的语句
                ...
                cmd.Transaction = tran; //设置命令对象的事务属性
                return tran;
            }
            catch (Exception ex)
            { //抛出异常
